...
import Control.Practica_Control;
...
public class GUI_Practica extends JDialog {
	...
	public GUI_Practica(Login l) {
		...
		tfTitulo.setBounds(102, 48, 86, 25);
		getContentPane().add(tfTitulo);
		tfTitulo.setColumns(10);
		
		cbSerie = new JComboBox();
		cbSerie.setBounds(102, 155, 71, 25);
	...
	public void Guardar(){
		...
		int codigo_practica = 1;
		String titulo = tfTitulo.getText().trim();
		int serie = Integer.parseInt(cbSerie.getSelectedItem().toString());
		String descripcion = tfDescripcion.getText().trim();
		
		Practica_Control rb = new Practica_Control();
		rb.addPractica(codigo_practica, serie, titulo, descripcion);
